...
CREATE TABLE `loadouts`(
	...
	`parent_id` CHAR(36), -- foreign key, the parent piece of gear, or null if top level
	`status` TINYINT NOT NULL, -- 0 = unpacked 1 = packed, other values for future use (not bringing, need to buy)
...
CREATE TABLE `xref_user_loadouts`(
	...
	`can_edit` BOOLEAN NOT NULL, -- whether this user has edit priveliges on this loadout
	`can_pack` BOOLEAN NOT NULL, -- whether this user has packing priveliges on this loadout
	`is_admin` BOOLEAN NOT NULL, -- can rename, delete, share, unshare, assign priveledges, etc.
...
-- this deletes the xref_user_loadouts table
-- DROP TABLE `xref_user_loadouts`;
...
-- 	`status` TINYINT NOT NULL -- whether this item is currently packed
